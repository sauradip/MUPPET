...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.w_qs = nn.Linear(d_model, n_head * d_k, bias=True).cuda()
        self.w_qs.requires_grad = True
        # print(self.w_qs)
        self.w_ks = nn.Linear(d_model, n_head * d_k, bias=True).cuda()
        # self.w_ks.requires_grad = True
        self.w_vs = nn.Linear(d_model, n_head * d_v, bias=True).cuda()
        ...
        self.layer_norm = nn.LayerNorm(d_model).cuda()
        # self.layer_norm = nn.BatchNorm1d(400).cuda()
    ...
    def forward(self, q, k, v):
        ...
        # output_1 = output + residual
        # output_1 = output_1.permute(0,2,1)
        # output = self.layer_norm(output_1).permute(0,2,1)
