...
class STALEEpisodicDataset(data.Dataset):
    ...
    def getVideoMaskcwise(self,video_annos,idx,sub_cls_dict,clip_length=100):
        ...
        if os.path.exists(os.path.join(self.feature_path+"/",idx+".npy")) and idx in list(video_annos.keys()):
            ...
            raise NotFoundErr("File not found, look for ", os.path.join(self.feature_path+"/",idx+".npy"))
    ...
    def loadVideo(self, video_name):
        ...
        frames = np.load(os.path.join(self.vid_path, video_name+".npy"))
    ...
    def getEpisodicVideoData(self,index):
        ...
        for class_chosen in rand_cls_list:
            ...
            query_data = self.loadVideo(rand_vid_name) ## backbone features
            ...
            for k in range(nshot):
                ...
                support_data = self.loadVideo(support_vname)
            ...
            spprt_vids = torch.cat(support_video_list,0).view(nshot,T,C,H,W)
            spprt_vids_trim = torch.cat(support_video_list_trim,0).view(nshot,T,C,H,W)
            cls_support_video.append(spprt_vids)
            cls_support_video_trim.append(spprt_vids_trim)
            cls_support_label_loc.append(torch.cat(support_label_list_loc,0).view(nshot,self.temporal_scale,self.temporal_scale))
            cls_support_label_class.append(torch.cat(support_label_list_class,0).view(nshot,-1))
            cls_support_label_mask.append(torch.cat(support_label_list_mask,0).view(nshot,nway+1,self.temporal_scale))
        ...
        support_data_tensor = torch.cat(cls_support_video,0).view(nway,nshot,C,T,H,W)  ### [nway=batch,nshot,feat_dim,temp]
        support_data_trim_tensor = torch.cat(cls_support_video_trim,0).view(nway,nshot,C,T,H,W)
        support_class_tensor = torch.cat(cls_support_label_class,0).view(nway,nshot,-1)
        support_loc_tensor = torch.cat(cls_support_label_loc,0).view(nway,nshot,self.temporal_scale,self.temporal_scale)
        support_mask_tensor = torch.cat(cls_support_label_mask,0).view(nway,nshot,nway+1,self.temporal_scale)
        ...
        query_loc_tensor = torch.cat(cls_query_label_loc,0).view(nway,self.temporal_scale,self.temporal_scale)
        query_mask_tensor = torch.cat(cls_query_label_mask,0).view(nway,nway+1,self.temporal_scale)
