...
def redundancy_loss(gt_action , pred_action, gt_cls, pred_cls, features):
    ...
    sim_loss = 0
    B,K,T = pred_cls.size()
    # print(' - debug:', pred_cls.size())
    pred_cls = pred_cls.view(int(B/nshot), nshot,K,T)
    
    pred_cls = torch.mean(pred_cls,dim=1)
    # print(pred_cls.size())

    btch,_,_ = pred_cls.size()
    if T == 100 :
        for i in range(btch):
            val_top,_ = torch.max(torch.softmax(pred_cls[i,:(K-1),:],dim=0),dim=0)
...
def top_branch_loss(gt_cls, pred_cls, mask_gt, cls_pred, cls_gt, features, mode):
    ...
    loss = 0.9*top_ce_loss(gt_cls.cuda(), pred_cls) 
...
def stale_loss(gt_cls, pred_cls ,gt_action , pred_action, mask_gt, bot_pred, bot_gt, cls_pred, cls_gt, features, mode):
    ...
    # print("mode", mode)
    # print(pred_cls.size(), gt_cls.size())
    # print(pred_action.size() , gt_action.size())
    ...
    if not meta_class : ## learn both class and mask branch
        if mode =="train":
            ...
            bg_loss = 0
            # tot_loss = top_loss + beta*bottom_loss + gamma*bg_loss + delta*fg_loss
            tot_loss = 0.4*top_loss + 0.3*bottom_loss + bg_loss + 0.3*fg_loss
        else:
            top_loss = top_branch_loss(gt_cls, pred_cls, mask_gt, cls_pred,cls_gt,features,mode)
            bottom_loss = bottom_branch_loss(gt_action.cuda(), pred_action) 
            # red_loss = redundancy_loss(gt_action , pred_action, gt_cls, pred_cls, features)
