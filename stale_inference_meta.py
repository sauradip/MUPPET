...
import stale_lib.stale_dataloader_base as stale_dataset
import stale_lib.stale_dataloader_fs as stale_dataset_fs
        ...
        for run in range(n_runs): ## epochs
            for e in tqdm(range(episodes)):
                ...
                for i in range(batch_size_val):
                    ...
                    nway,nshot,C,T = support_data.size()
                    support_top_gt = support_dict['class_branch'][0].view(-1,T)
                    support_bot_gt = support_dict['mask_branch'][0].view(-1,T,T)
                    support_1d_gt = support_dict['1d_mask'][0].view(-1,T)
                    support_class_bin = support_dict['class_branch_bin'][0].view(-1,nway+1,T)
                    support_class_gt = support_dict['class_1d_gt'].view(-1,nway)

                    query_data = query_dict['data'][0].view(-1,C,T)
                    query_top_gt = query_dict['class_branch'][0].view(-1,T)
                    query_bot_gt = query_dict['mask_branch'][0].view(-1,T,T)
                    query_1d_gt = query_dict['1d_mask'][0].view(-1,T)
                    query_class_bin = query_dict['class_branch_bin'][0].view(-1,nway+1,T)
                    ...
                    support_data = support_data.view(-1,C,T)
                    support_data_nshot = support_data.view(-1,nshot,C,T) # [nway,nshot,C,T]
                    support_1d_gt_nshot = support_1d_gt.view(-1,nshot,T) # [nway,nshot,T]#
                    support_1d_gt_nshot = support_1d_gt_nshot.unsqueeze(2).expand(-1,-1,C,-1) # [nway,nshot,C,T]
